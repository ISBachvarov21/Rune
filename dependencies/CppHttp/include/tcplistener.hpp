...
public:
  ...
  void Accept() {
    ...
    SOCKET newConnection = accept(listener, (struct sockaddr *)&this->server,
                                  (socklen_t *)&this->serverLen);
#endif
    ...
    tasks.push([this, newConnection]() {
      ...
      tv.tv_sec = 0;
      tv.tv_usec = 100000;
      setsockopt(newConnection, SOL_SOCKET, SO_RCVTIMEO, (const char *)&tv,
                 sizeof tv);
      ...
      std::string contentLength = CppHttp::Utils::GetHeader(
          std::string(buffer.get()), "Content-Length");

#pragma region If content length is specified, continue reading until the body is fully received
      if (contentLength != "") {
        do {
          int bytesReceivedTemp =
              recv(newConnection, &buffer.get()[bytesReceived], 65535, 0);
        ...
        } while (CppHttp::Utils::GetBody(std::string(buffer.get())).length() <
                 std::stoi(contentLength));
      }
#pragma endregion
      ...
      req.m_info.uoriginal =
          std::u8string(buffer.get(), buffer.get() + bytesReceived);
